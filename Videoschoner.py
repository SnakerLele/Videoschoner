...
def get_primary_monitor():
    # Hole Informationen über den Hauptbildschirm mit screeninfo
    monitors = get_monitors()
    primary_monitor = None
    
    # Suche nach dem Hauptbildschirm
    for monitor in monitors:
        if monitor.is_primary:
            primary_monitor = monitor
            break
    
    # Falls kein Hauptbildschirm gefunden wurde, nimm den ersten
    if primary_monitor is None and monitors:
        primary_monitor = monitors[0]
    
    if primary_monitor is None:
        raise Exception("Kein Monitor gefunden")
...
def main():
    ...
    # Hole die Informationen des Hauptbildschirms
    primary_monitor = get_primary_monitor()
    
    # Erstelle das Fenster auf dem Hauptbildschirm
    os.environ['SDL_VIDEO_WINDOW_POS'] = f"{primary_monitor['left']},{primary_monitor['top']}"
    screen = pygame.display.set_mode((primary_monitor['width'], primary_monitor['height']), pygame.FULLSCREEN)
